...
const buildBasePath = ({ tourId, userId, isPrivate = false, pathOverride }) => {
  if (pathOverride) {
    return pathOverride;
  }

  if (isPrivate) {
    return `private_tour_photos/${tourId}/${userId}`;
  }
...
const loadTourPhotos = async (
  ...
  try {
    if (isPrivate && !userId && !path) {
      const missingError = new Error('Missing user for private photos');
      missingError.code = 'invalid-params';
      throw missingError;
    }

    const basePath = buildBasePath({ tourId, userId, isPrivate, pathOverride: path });
    const tourPhotosRef = storageRefFn(storageInstance, basePath);
...
const uploadImage = async (
  ...
  try {
    ...
    if (isPrivate && !userId && !path) {
      const missingUserError = new Error('Missing user for private upload');
      missingUserError.code = 'invalid-params';
