...
test('group media access accepts only a current passenger or coherently assigned driver', async () => {
  ...
  assert.deepEqual(await __testables.verifyCurrentTourPhotoAccess({ db, authUid: 'passengerA', tourId: 'TOUR_A' }), {
    allowed: true, role: 'passenger', principalId: passengerId,
  });
  assert.equal((await __testables.verifyCurrentTourPhotoAccess({ db, authUid: 'passengerA', tourId: 'TOUR_B' })).allowed, false);
  assert.deepEqual(await __testables.verifyCurrentTourPhotoAccess({ db, authUid: 'driverA', tourId: 'TOUR_A' }), {
    allowed: true, role: 'assigned_driver', principalId: 'driver:D-1', driverId: 'D-1',
  });
