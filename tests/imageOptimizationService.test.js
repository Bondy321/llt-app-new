...
test('optimizeImageForUpload converges with adaptive loop and reports metrics', async () => {
  const service = buildService({
    sizeByCall: (uri) => {
      if (uri.includes('optimized-1')) return 2_000_000;
      if (uri.includes('optimized-2')) return 1_200_000;
      if (uri.includes('optimized-3')) return 150_000;
      if (uri.includes('optimized-4')) return 100_000;
      return 2_400_000;
    },
  });
  ...
  assert.equal(result.thumbnailUri, 'file://optimized-4.jpg');
  ...
  assert.equal(typeof result.metrics.optimizationRatio, 'number');
});

test('optimizeImageForUpload exits safely at max iterations when target budget is never met', async () => {
