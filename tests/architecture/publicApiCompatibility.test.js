...
test('high-value HTTP reason codes remain represented at the backend boundary', () => {
  ...
  const expectedReasonCodes = [
    ...
    'ORIGIN_NOT_ALLOWED', 'PHOTO_NOT_FOUND', 'SERVICE_UNAVAILABLE', 'SESSION_CHANGED',
  ...
  for (const code of expectedReasonCodes) assert.match(source, new RegExp(`reason: ['\"]${code}['\"]`, 'u'), code);
});
