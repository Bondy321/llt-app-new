...
1. **Firebase region:** backend resources run in `europe-west1` by default.
   - **Exception:** `generatePhotoVariants` runs in `us-east1` because Cloud Storage triggers must match bucket region, and the default Firebase free-tier bucket is `us-east1`.
